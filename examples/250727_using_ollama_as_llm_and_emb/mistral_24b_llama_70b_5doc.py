...
async def cheap_model_if_cache(
    ...
    if hashing_kv is not None:
        args_hash = compute_args_hash(Cheap_model, messages)
        if_cache_return = await hashing_kv.get_by_id(args_hash)
        if if_cache_return is not None:
            return if_cache_return["return"]
    # -----------------------------------------------------
    response = await ollama_client.chat(model=Cheap_model, messages=messages, **kwargs)

    result = response["message"]["content"]
    # Cache the response if having-------------------
    if hashing_kv is not None:
        await hashing_kv.upsert({args_hash: {"return": result, "model": Cheap_model}})
    # -----------------------------------------------------
    return result


async def thinking_model_if_cache(
    prompt, system_prompt=None, history_messages=[], **kwargs
) -> str:
    # remove kwargs that are not supported by ollama
    kwargs.pop("max_tokens", None)
    ...
    messages.append({"role": "user", "content": prompt})
    if hashing_kv is not None:
        args_hash = compute_args_hash(Thinking_model, messages)
        if_cache_return = await hashing_kv.get_by_id(args_hash)
        if if_cache_return is not None:
            return if_cache_return["return"]
    # -----------------------------------------------------
    response = await ollama_client.chat(model=Thinking_model, messages=messages, **kwargs)

    result = response["message"]["content"]
    # Cache the response if having-------------------
    if hashing_kv is not None:
        await hashing_kv.upsert({args_hash: {"return": result, "model": Thinking_model}})
    # -----------------------------------------------------
    return result
...
WORKING_DIR = "./250801_mistral_24b_llama_70b"
...
def query(question=None):
    rag = GraphRAG(
        ...
        best_model_func=thinking_model_if_cache,
        cheap_model_func=cheap_model_if_cache,
...
def insert():
    ...
    # FAKE_TEXT = f.read()
    texts = []
    doc_dir = "/data/hallucination/250726_muyu/250727_nano_graphrag/nano-graphrag/250710_documents_5"
    ...
    rag = GraphRAG(
        ...
        best_model_func=cheap_model_if_cache,
        cheap_model_func=cheap_model_if_cache,
        embedding_func=ollama_embedding,
    )
    start = time()
...
if __name__ == "__main__":
    ...
    with io.open (f"/data/hallucination/250726_muyu/250727_nano_graphrag/nano-graphrag/examples/250727_using_ollama_as_llm_and_emb/mistral_24b_5doc.json", "w") as f:
